				...
				<tbody>
					...
					if (isset($_GET["searched"])) {
						if ($_GET["searched"] == "click") {
							...
							$query = $connection->query("SELECT * FROM kost a WHERE nama LIKE '%$_GET[nama]%' AND status='$_GET[status]'");
						}
					} else {
						$query = $connection->query("SELECT * FROM kost ORDER BY harga_3bulan");
					}
					$no = 1;
					?>
